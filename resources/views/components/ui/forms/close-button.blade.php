@props(['showClose' => '','wireFunction'=>''])
@if($showClose)
  <button class="btn btn-error w-full mt-4 " wire:click="{{$wireFunction}}">
    {{__('Close')}}
    {{-- <span wire:loading wire:loading.delay.long>.......</span> --}}
  </button>
@else
    <button class="btn btn-error w-full mt-4 text-error " disabled  wire:target="{{$wireFunction}}">
